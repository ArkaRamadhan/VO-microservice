...
func ImportExcelPerdin(c *gin.Context) {
	...
	for i, row := range rows {
		...
		noPerdin := initializers.GetStringOrNil(initializers.GetColumn(row, 0))
		tanggalStr := initializers.GetStringOrNil(initializers.GetColumn(row, 1))
		hotel := initializers.GetStringOrNil(initializers.GetColumn(row, 2))
		transport := initializers.GetStringOrNil(initializers.GetColumn(row, 3))
